-- =====================================================================
-- Migration: o toggle `ativo` do RECURSO passa a controlar o cron
--
-- Decisao (09/06, "completo"): recursos.<recurso>.ativo vira o MASTER SWITCH
-- do modulo. Ate aqui dois flags independentes governavam o mesmo job pg_cron:
--   - recursos.<recurso>.agendamento.ativo (relogio, ligado pelo card Agendamento)
--   - recursos.<recurso>.ativo             (modulo, ligado pelo card de Config)
-- Resultado: dava para ter o modulo DESLIGADO mas o cron AINDA disparando.
--
-- Esta migration faz o ativo do recurso ser pre-requisito do cron: recurso
-- desligado => NENHUM job, mesmo com agendamento.ativo=true. O backend
-- (nomus-ingerir.loadTiposAtivos) ja ganhou a defesa correspondente (lista
-- vazia quando ativo===false), entao o disparo manual tambem respeita o gate.
--
-- A flag `ativo` AUSENTE preserva o legado (trata como ligado) — so o valor
-- explicito `false` desliga. Idempotente (create or replace). Aplicar via Node
-- `pg` (SUPABASE_DB_URL), o caminho padrao deste projeto.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Re-aplica o agendamento do Nomus/processos para o gate valer ja: se o
-- recurso estiver desligado o job 'coleta-nomus-processos' some agora.
-- ---------------------------------------------------------------------
do $reapply$
declare
  v_res text;
begin
  if exists (select 1 from public.fontes where tipo = 'nomus') then
    ...
    raise notice 'reapply nomus/processos pos-gate: %', v_res;
  end if;
end;
$reapply$;
