...
// 2026-06-09: conta separada). Espelha drive-oauth, mas com refresh_token
// proprio no Vault (GMAIL_REFRESH_TOKEN), tabelas proprias (gmail_conta /
// gmail_oauth_state) e callback proprio. Reusa o MESMO Client Web do
// login/Drive (mesmo app OAuth) — a "conta separada" vem do refresh_token
// distinto, nao de um app novo. Escopo: gmail.readonly (so leitura).
//
...
//                     MUDOU, limpa gmail_labels. Grava o refresh_token CIFRADO
//                     no Vault e registra a conta em gmail_conta. Redireciona
//                     o navegador de volta ao cockpit (?gmail=conectado|erro).
...
const OAUTH_SCOPE = `openid email ${GMAIL_SCOPE}`;
...
async function handleIniciar(req: Request): Promise<Response> {
  ...
  const { error } = await service
    ...
    .insert({ state, email });
  if (error) {
    throw new HttpError(500, "state_write_failed", "falha ao iniciar a conexao com o Gmail");
  }
...
async function consumirState(state: string): Promise<string | null> {
  ...
  await service.from("gmail_oauth_state").delete().eq("state", state);
...
async function handleCallback(req: Request): Promise<Response> {
  ...
  // navegador ao cockpit com ?gmail=erro, nunca despejar JSON cru na tela. O
  ...
  try {
    ...
    // Conta anterior: se MUDOU, limpa as labels cadastradas (espelha o Drive,
    // que limpa as pastas — a blacklist de labels e por conta).
    ...
    if (trocouConta) {
      await service.from("gmail_labels").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    }

    // Grava o refresh_token CIFRADO no Vault (nunca persiste em coluna).
    await setServiceSecret(GMAIL_REFRESH_NAME, refreshToken);
    ...
    if (upErr) {
      throw new HttpError(500, "gmail_conta_upsert_failed", "falha ao registrar a conta do Gmail");
    }
    ...
    await logSensitiveAction({
      ...
      acao: "conectar_gmail",
      usuario: iniciadoPor,
      dadosNovos: { email, contaTrocada: trocouConta, labelsLimpas: trocouConta },
    });

    return redirectCockpit(cfg.returnUrl, "conectado");
  } catch (err) {
    console.error("[gmail-oauth] callback falhou:", err);
    return redirectCockpit(cfg.returnUrl, "erro");
  }
}
...
async function handleAccessToken(req: Request): Promise<Response> {
  ...
  const refreshToken = await getServiceSecret(GMAIL_REFRESH_NAME);
  if (!refreshToken) {
    throw new HttpError(409, "gmail_nao_conectado", "Gmail nao conectado: conecte a conta no cockpit");
  }
  ...
  const text = await res.text();
  if (!res.ok) {
    throw new HttpError(502, "oauth_refresh_failed", "falha ao renovar o access_token do Gmail");
  }
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    return await errorResponse(err, { fn: "gmail-oauth" });
  }
}
