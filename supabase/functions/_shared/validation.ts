...
const relacionamentoSequenciaSchema = z
  .array(
    z
...
const catalogoRegraBaseSchema = z.object({
  ...
  sequencia: relacionamentoSequenciaSchema,
...
export const catalogoRegraIdSchema = z
  .object({
    id: z.string({ invalid_type_error: "id deve ser string" }).uuid("id invalido"),
  })
  .strict();
  ...
  .superRefine((val, ctx) => {
    if (val.combinacao === "simples" && val.campo_destino === "numero_pregao") {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["campo_destino"],
        message: REL_NUMERO_PREGAO_REFINADO_MSG,
      });
    }
  });

export type CatalogoRegraCreateInput = z.infer<typeof catalogoRegraCreateSchema>;

export const catalogoRegraUpdateSchema = catalogoRegraBaseSchema
  .partial()
  .strict()
  .superRefine((val, ctx) => {
    if (val.combinacao === "simples" && val.campo_destino === "numero_pregao") {
...
export type CatalogoRegraUpdateInput = z.infer<typeof catalogoRegraUpdateSchema>;
...
export const vinculoLiaCreateSchema = z
  .object({
    ...
    contador_2caminhos: z
      ...
      .optional(),
    regra_macro_id: z.string().uuid("regra_macro_id invalido").nullable().optional(),
    motivo: z
      .string()
      .trim()
...
export type VinculoLiaCreateInput = z.infer<typeof vinculoLiaCreateSchema>;
...
export type VinculoLiaUpdateInput = z.infer<typeof vinculoLiaUpdateSchema>;
...
export const vinculoLiaDecidirSchema = z
  ...
  .strict()
  .superRefine((val, ctx) => {
    if (val.acao !== "aprovar" && (val.motivo === undefined || val.motivo.trim() === "")) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["motivo"],
...
export type VinculoLiaDecidirInput = z.infer<typeof vinculoLiaDecidirSchema>;

export const configRelacionamentosUpdateSchema = z
  .object({
...
export type RelacionamentosVizinhancaPayload = z.infer<
  typeof relacionamentosVizinhancaPayloadSchema
>;

export const v1RelacionamentosNoPayloadSchema = z
  .object({
    tipo: relacionamentoTipoNoEnum,
    id: z
      .string({ required_error: "id e obrigatorio", invalid_type_error: "id deve ser string" })
...
export type V1RelacionamentosNoPayload = z.infer<typeof v1RelacionamentosNoPayloadSchema>;

export const v1RelacionamentosVizinhosPayloadSchema = z
  .object({
    tipo: relacionamentoTipoNoEnum,
    id: z
      .string({ required_error: "id e obrigatorio", invalid_type_error: "id deve ser string" })
    ...
    profundidade: z
      .number({ invalid_type_error: "profundidade deve ser numero" })
      .int("profundidade deve ser inteiro")
      .min(1, "profundidade deve ser >= 1")
      .max(5, "profundidade deve ser <= 5")
      .optional(),
  })
  .strict();

export type V1RelacionamentosVizinhosPayload = z.infer<
  typeof v1RelacionamentosVizinhosPayloadSchema
>;

export const v1RelacionamentosBuscarPayloadSchema = z
  .object({
    consulta: z
      .string({
