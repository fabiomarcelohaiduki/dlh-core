...
import type { SalvarAgendamentoExtracaoInput } from "@/lib/api/admin";
import type { AgendamentoExtracaoState } from "@/lib/api/types";

/**
 * Frequencias de um relogio do cockpit. 'manual' (desligado) e expresso pelo
...
/**
 * Schema cliente (espelha os schemas de agendamento do backend). horario em
...
/** Mutation minima aceita pelo form (qualquer useMutation de agendamento). */
interface AgendamentoMutation {
  mutateAsync: (input: SalvarAgendamentoExtracaoInput) => Promise<unknown>;
  isPending: boolean;
}

export interface AgendamentoCronFormProps {
  initial: AgendamentoExtracaoState;
  /** Mutation que persiste o agendamento (PUT do endpoint correspondente). */
  salvar: AgendamentoMutation;
  /** Rotulo do toggle "ligado" (ex.: "Extração automática ligada"). */
  ativoLabel: string;
  /** Texto auxiliar sob o campo Frequência (o que o relogio faz). */
  helperFrequencia: string;
  /** Horario default ao ligar quando ainda nao ha um salvo (HH:MM local). */
  defaultHorario: string;
  /** Mensagens de sucesso ao salvar (ligado / desligado). */
  okMessageOn: string;
  okMessageOff: string;
}

function toDefaults(initial: AgendamentoExtracaoState, defaultHorario: string): AgValues {
  const freq = initial.frequencia === "manual" ? "diaria" : initial.frequencia;
  return {
    ativo: initial.ativo,
    frequencia: freq,
    horarioReferencia: initial.horarioReferencia ?? defaultHorario,
...
/**
 * cmp-agendamento-cron-form — relogio generico de agendamento do cockpit.
 *
 * Compartilhado entre os agendamentos que tem a MESMA forma (ativo + frequencia
 * + horario + dia da semana/mes) e gravam um pg_cron via PUT: a Extracao
 * (Tika/OCR) e a Descoberta/enfileiramento do Nomus. O toggle "ativo"
 * liga/desliga; frequencia + horario definem a cadencia. Salvar chama a
 * `salvar` recebida, que reescreve o pg_cron no substrato (sem redeploy). A
 * copy (rotulo do toggle, helper, mensagens) vem por prop para o card de cada
 * dominio falar a sua lingua.
 */
export function AgendamentoCronForm({
  initial,
  salvar,
  ativoLabel,
  helperFrequencia,
  ...
  const {
    ...
    defaultValues: toDefaults(initial, defaultHorario),
  });
  ...
  async function onSubmit(values: AgValues) {
    ...
    try {
      ...
      reset(values);
      setFeedback({ kind: "ok", message: values.ativo ? okMessageOn : okMessageOff });
    } catch (err) {
      const message =
  ...
  return (
    <form className="card form-card" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      >
        ...
        />
        <div className="t">{ativoLabel}</div>
      </label>
        ...
        <>
          <div className="grid-fields">
            <div className="field">
              ...
              <div className="helper">{helperFrequencia}</div>
            </div>
      ...
      <div className="form-foot" style={{ marginTop: 22 }}>
        ...
        <button
          ...
          type="button"
          onClick={() => {
            reset(toDefaults(initial, defaultHorario));
            setFeedback(null);
          }}
