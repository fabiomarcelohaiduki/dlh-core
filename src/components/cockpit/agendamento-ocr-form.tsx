...
import { useSalvarAgendamentoOcr } from "@/hooks/use-admin";
import { ApiError } from "@/lib/api/client";
import { cn } from "@/lib/utils";
import type { AgendamentoExtracaoState } from "@/lib/api/types";

/**
 * Frequencias do OCR automatico. 'manual' (desligado) e expresso pelo proprio
 * toggle "ativo"; nao aparece como opcao de frequencia.
 */
const FREQUENCIAS = [
...
/**
 * Schema cliente (espelha extracaoAgendamentoSchema do backend, reusado pelo
 * ocr-agendamento). horario em 'HH:MM' local; diaSemana 0..6; diaMes 1..28.
 */
...
function toDefaults(initial: AgendamentoExtracaoState): AgValues {
  ...
  return {
    ativo: initial.ativo,
    frequencia: freq,
    horarioReferencia: initial.horarioReferencia ?? "01:00",
...
/**
 * cmp-agendamento-ocr-form — Agendamento do EXTRATOR OCR (passo dedicado).
 *
 * O OCR drena SO a fila 'precisa_ocr' (escaneados/imagem) com OCR ligado,
 * separado do extrator rapido. O toggle "ativo" liga/desliga a drenagem
 * automatica; frequencia + horario definem a cadencia. Salvar reescreve o
 * pg_cron 'extrair-ocr' no substrato (sem redeploy) via PUT /ocr-agendamento.
 * Como OCR e caro, recomenda-se medir 1 run manual antes de ligar.
 */
export function AgendamentoOcrForm({ initial }: { initial: AgendamentoExtracaoState }) {
  const salvar = useSalvarAgendamentoOcr();
    ...
    try {
      ...
      setFeedback({
        ...
        message: values.ativo
          ? "Agendamento salvo · OCR automático ligado"
          : "Agendamento salvo · OCR automático desligado",
      });
    } catch (err) {
      const message =
  ...
  return (
    <form className="card form-card" onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="grid-fields">
          <div className="field">
            <label htmlFor="ocr-freq">Frequência</label>
            <select id="ocr-freq" {...register("frequencia")}>
              {FREQUENCIAS.map((f) => (
            ...
            <div className="helper">Com que frequência o OCR drena a fila de escaneados.</div>
          </div>

          <div className={cn("field", errors.horarioReferencia && "invalid")}>
            <label htmlFor="ocr-hora">Horário</label>
            <input
              type="time"
              id="ocr-hora"
        ...
        {frequencia === "semanal" && (
          <div className="field" style={{ maxWidth: 300 }}>
            <label htmlFor="ocr-dow">Dia da semana</label>
            <select id="ocr-dow" {...register("diaSemana", { valueAsNumber: true })}>
              {DIAS_SEMANA.map((d) => (
        ...
        {frequencia === "mensal" && (
          <div className={cn("field", errors.diaMes && "invalid")} style={{ maxWidth: 300 }}>
            <label htmlFor="ocr-dom">Dia do mês</label>
